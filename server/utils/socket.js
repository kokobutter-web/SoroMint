...
const initEmitter = () => {
    ...
    if (redisUrl) {
        try {
            ...
            logger.error('Failed to initialize Socket.io Redis emitter', { error: error.message });
        }
    }
    return null;
};
...
const emitEvent = (event, data, room = null) => {
    try {
        ...
        if (emitter) {
            ...
            logger.debug('Socket emission skipped: neither io nor emitter available', { event });
        }
    } catch (error) {
